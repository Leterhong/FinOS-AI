...
export default function ModelsPage() {
  ...
  return <div className="page-shell">
    ...
    <Panel>
      ...
      {loading ? <div className="flex items-center justify-center gap-2 p-12 text-xs text-slate-500"><Loader2 className="h-4 w-4 animate-spin" />正在读取加密配置</div> : models.length === 0 ? <div className="p-12 text-center"><ServerCog className="mx-auto h-8 w-8 text-slate-700" /><p className="mt-3 text-sm text-slate-300">没有模型配置</p><p className="mt-2 text-xs text-slate-600">添加一个模型后，所有 AI 页面才会开放真实调用。</p></div> : <div className="divide-y divide-white/[0.06]">{models.map((model) => { const meta = statusMeta[model.status]; return <article key={model.id} className="grid gap-4 p-5 sm:grid-cols-[1fr_.65fr_auto] sm:items-center"><div className="min-w-0"><div className="flex items-center gap-2"><h3 className="truncate text-sm font-semibold text-slate-100">{model.displayName}</h3>{model.isDefault && <span className="inline-flex items-center gap-1 rounded-md border border-amber-400/15 bg-amber-400/[0.05] px-2 py-1 text-[9px] text-amber-200"><Star className="h-3 w-3 fill-current" />默认</span>}</div><p className="mt-1.5 truncate font-mono text-[11px] text-slate-500">{model.modelId} · {model.keyMask}</p><p className="mt-1 truncate text-[10px] text-slate-600">{model.baseUrl}</p></div><div><div className="flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${meta.dot}`} /><span className={`text-xs ${meta.className}`}>{meta.label}</span></div><p className="mt-1.5 text-[10px] text-slate-600">{PROVIDER_PRESETS[model.providerType]?.label ?? model.providerType}{model.lastLatencyMs != null ? ` · ${model.lastLatencyMs} ms` : ""}</p></div><div className="flex flex-wrap items-center gap-2"><button onClick={() => void handleTest(model.id)} disabled={testing === model.id} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-white/[0.08] px-3 text-[10px] text-slate-300 disabled:opacity-40">{testing === model.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <Activity className="h-3 w-3" />}测试</button>{!model.isDefault && <button onClick={() => void handleDefault(model.id)} className="grid h-9 w-9 place-items-center rounded-lg border border-white/[0.08] text-slate-400" title="设为默认"><Star className="h-3.5 w-3.5" /></button>}<button onClick={() => { setEditing(model); setDialogOpen(true); }} className="grid h-9 w-9 place-items-center rounded-lg border border-white/[0.08] text-slate-400" title="编辑"><Pencil className="h-3.5 w-3.5" /></button><button onClick={() => void handleDelete(model)} className="grid h-9 w-9 place-items-center rounded-lg border border-rose-400/10 text-rose-300/70" title="删除"><Trash2 className="h-3.5 w-3.5" /></button></div></article>; })}</div>}
    </Panel>

    <div className="grid gap-3 md:grid-cols-3">{[[KeyRound,"密钥服务端加密","API Key 不返回浏览器，只展示掩码"],[ShieldCheck,"按工作区隔离","每个浏览器工作区拥有独立配置文件"],[Zap,"统一模型协议","Agent 与助手使用当前默认模型"]].map(([Icon,title,text]) => { const ItemIcon = Icon as typeof KeyRound; return <div key={String(title)} className="rounded-xl border border-white/[0.07] bg-white/[0.02] p-4"><ItemIcon className="h-4 w-4 text-cyan-300" /><p className="mt-3 text-xs font-medium text-slate-200">{String(title)}</p><p className="mt-1.5 text-[10px] leading-5 text-slate-600">{String(text)}</p></div>; })}</div>
