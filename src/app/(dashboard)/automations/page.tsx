      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {/* 始终渲染 <p> 元素，仅在无 desc 时用 hidden 隐藏，
                避免 SSR/客户端条件分支不同导致的 hydration mismatch。 */}
            <p
              className={cn("text-xs text-white/40", !desc && "hidden")}
              aria-hidden={!desc || undefined}
            >
              {desc || ""}
            </p>
          </div>
...
/**
 * 偏好画像维度值的轻量渲染：避免直接 JSON.stringify 带 2 空格缩进溢出
 * 卡片宽度被迫出现横向滚动条（与暗色主题严重不协调）。
 * - object：键值对行式展示，长 value 自动换行
 * - primitive：直接展示
 * - null/undefined：占位 —
 */
function renderDimensionValue(v: unknown) {
      ...
      <div className="space-y-0.5">
        {entries.map(([k, val]) => (
          <div key={k} className="flex gap-2 break-words">
            ...
            <span className="min-w-0 text-white/70">{formatPrimitive(val)}</span>
          </div>
        ))}
      </div>
    );
  }
  return <span className="text-white/70">{formatPrimitive(v)}</span>;
}
...
export default function AutomationsPage() {
  ...
  // 客户端首帧守卫：避免 useAutonomousOverview 在 SSR 端 enabled=false、
  // 客户端 enabled=true 导致 isLoading 不同、进而 conditional 渲染不同分支
  // 触发 hydration mismatch。SSR 与客户端首次绘制统一渲染骨架，挂载后才
  // 切到真实数据（state update 而非 hydration 重建子树）。
  ...
  return (
    <div className="space-y-6">
      ...
      {!mounted || isLoading ? (
        ...
        <Section title="AI 控制中心" icon={Cpu} desc="连接失败">
          ...
          {data!.map((s: AutomationSchedule) => (
            <li key={s.id} className="flex items-center gap-3 rounded-xl border border-white/8 bg-white/[0.02] p-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-white/40">{s.taskTypeLabel} · {s.frequency} · 下次 {s.nextRunAt ? new Date(s.nextRunAt).toLocaleString("zh-CN", { hour12: false }) : "—"}</p>
              </div>
          ...
          {data!.map((p: AutomationPlan) => (
            <li key={p.id} className="flex items-center gap-3 rounded-xl border border-white/8 bg-white/[0.02] p-3">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-white/40">{p.agentLabel} · {p.cadence} · 下次 {p.nextRunAt ? new Date(p.nextRunAt).toLocaleString("zh-CN", { hour12: false }) : "—"}</p>
          ...
          {(data.dimensions ?? []).map((d) => (
            <div key={d.dimension} className="rounded-xl border border-white/8 bg-white/[0.02] p-3">
              <div className="flex items-center justify-between gap-2">
                <span className="truncate text-sm font-medium text-white">{d.dimension}</span>
                <span className="shrink-0 text-xs text-white/40">置信度 {Math.round((d.confidence || 0) * 100)}% · 样本 {d.sampleCount}</span>
              </div>
              <div className="mt-1.5 text-[11px] leading-relaxed">
                {renderDimensionValue(d.value)}
              </div>
            </div>
          ))}
          {bias.data && (
