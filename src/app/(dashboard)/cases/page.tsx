...
export default function CasesPage() {
  ...
  const cases = useMemo(() => items.filter(item => (risk === "全部" || item.risk === risk) && `${item.company}${item.title}${item.industry}`.toLowerCase().includes(query.toLowerCase())), [items, query, risk]);
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    setCreateOpen(false); setSelected(item);
  };
