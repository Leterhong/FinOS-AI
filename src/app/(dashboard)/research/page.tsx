...
export default function ResearchPage() {
  ...
  ), [activeCaseId, briefs, cases.length]);

  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!active?.configured || !activeCase || generating) return;
  ...
  return <div className="page-shell">
    <PageIntro eyebrow="AI research workspace" title="企业投研中心" description="使用当前默认模型基于所选企业项目生成研究框架和底稿。未连接外部数据源时，AI 会列出需要补充的来源，不会伪造行情、政策或新闻。" actions={<button onClick={() => setOpen(true)} disabled={!active?.configured || !activeCase} className="inline-flex items-center gap-2 rounded-xl bg-cyan-300 px-4 py-2.5 text-xs font-semibold text-[#041018] disabled:cursor-not-allowed disabled:opacity-40"><Sparkles className="h-3.5 w-3.5" />生成 AI 研究</button>} />
    <CaseContextSelector cases={cases} value={activeCaseId} onChange={setActiveCaseId} detail={`${caseDocuments.length} 份资料 · ${caseBriefs.length} 份底稿，研究上下文按项目隔离`} />
