...
/**
 * 结构化答案渲染：模型按简报要求输出编号章节（概要/事实/规则/风险/复核清单），
 * 这里按章节拆分渲染，让「回答 / 关键发现 / 建议下一步」在视觉上分区；
 * 没有章节结构的内容优雅降级为纯文本。
 */
const SECTION_RE = /^\s*(?:(\d+)[.、．]|([一二三四五六七八九十]+)[、．.]|#{1,3})\s*(.+)$/;
...
function parseSections(content: string): Array<{ title: string; body: string }> {
  ...
  for (const line of lines) {
    ...
    if (match && line.trim().length < 60) {
      if (current) sections.push({ title: current.title, body: current.body.join("\n") });
      current = { title: (match[1] ? `${match[1]}. ` : "") + (match[3] ?? match[2] ?? "").trim(), body: [] };
      continue;
    }
  ...
  return (
    <div className="space-y-4">
      {sections.map((section, index) => (
        <section key={index}>
          <p className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.14em] text-cyan-300/70">
            <span className="grid h-4 w-4 place-items-center rounded border border-cyan-400/20 text-[9px]">{index + 1}</span>
            {section.title}
          </p>
          <p className="mt-2 whitespace-pre-wrap text-sm leading-7">{section.body.trim() || "—"}</p>
        </section>
      ))}
    </div>
  );
      ...
      <Panel className="flex min-h-[650px] flex-col">
        {!activeCase ? <EmptyStateCard icon={DatabaseZap} className="flex-1" title="先选择一个企业项目" description="研判对话必须绑定明确企业，避免不同客户或项目的数据进入同一次模型请求。" action={<Link href="/cases" className="rounded-xl bg-cyan-300 px-4 py-2.5 text-xs font-semibold text-[#041018]">创建企业项目</Link>} /> : !active?.configured ? <EmptyStateCard icon={Cpu} className="flex-1" title="先连接一个 AI 大模型" description="助手不会使用本地模板或伪造回复。请在模型中心填写 Provider、Base URL、Model ID 和 API Key，并完成连接测试。" action={<Link href="/models" className="rounded-xl bg-cyan-300 px-4 py-2.5 text-xs font-semibold text-[#041018]">前往 AI 模型中心</Link>} /> : <>
          <div className="scrollbar-thin flex-1 overflow-y-auto p-4 sm:p-6">
            {messages.length === 0 ? <div className="flex h-full min-h-80 flex-col items-center justify-center text-center"><div className="grid h-14 w-14 place-items-center rounded-xl border border-cyan-400/15 bg-cyan-400/[0.06]"><Sparkles className="h-6 w-6 text-cyan-300" /></div><h2 className="mt-4 text-base font-semibold text-white">研判 {activeCase.company}</h2><p className="mt-2 max-w-lg text-xs leading-6 text-slate-500">当前上下文限定为 1 个项目、{caseDocuments.length} 份资料、{rules.length} 条全局规则和 {caseRisks.length} 个项目风险。数据不足时，模型会明确提示需要补充数据。</p><div className="mt-5 flex max-w-2xl flex-wrap justify-center gap-2">{promptStarters.map((prompt) => <button key={prompt} onClick={() => setQuery(prompt)} className="rounded-xl border border-white/[0.08] bg-white/[0.025] px-3 py-2 text-[10px] text-slate-400 hover:border-cyan-400/20 hover:text-cyan-200">{prompt}</button>)}</div></div> : <div className="mx-auto max-w-3xl space-y-5">{messages.map((message) => <div key={message.id} className={`flex gap-3 ${message.role === "user" ? "justify-end" : "justify-start"}`}>{message.role === "assistant" && <div className="grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-cyan-400/15 bg-cyan-400/[0.07]"><Bot className="h-4 w-4 text-cyan-300" /></div>}<div className={`max-w-[88%] rounded-xl px-4 py-3 ${message.role === "user" ? "rounded-tr-sm bg-white/[0.07] text-slate-200" : message.error ? "rounded-tl-sm border border-rose-400/15 bg-rose-400/[0.04] text-rose-100" : "rounded-tl-sm border border-white/[0.07] bg-sheet text-slate-300"}`}>{message.role === "assistant" && !message.error ? <AnswerBody content={message.content} /> : <p className="whitespace-pre-wrap text-sm leading-7">{message.content}</p>}{message.role === "assistant" && !message.error && <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 border-t border-white/[0.06] pt-2 text-[9px] text-slate-600"><span>{message.model}</span><span>需人工复核</span></div>}</div>{message.role === "user" && <div className="grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-white/10 bg-white/[0.04]"><UserRound className="h-4 w-4 text-slate-400" /></div>}</div>)}</div>}
            {sending && <div className="mx-auto mt-5 max-w-3xl"><div className="flex gap-3"><div className="grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-cyan-400/15 bg-cyan-400/[0.07]"><Bot className="h-4 w-4 text-cyan-300" /></div><div className="max-w-[88%] rounded-xl rounded-tl-sm border border-white/[0.07] bg-sheet px-4 py-3 text-sm leading-7 text-slate-300">{streamText ? <p className="whitespace-pre-wrap">{streamText}<span className="ml-0.5 inline-block h-4 w-[2px] animate-pulse bg-cyan-300 align-middle" /></p> : <span className="inline-flex items-center gap-2 text-xs text-slate-400"><Loader2 className="h-3.5 w-3.5 animate-spin text-cyan-300" />模型正在研判工作区上下文</span>}</div></div></div>}
          </div>
          <div className="border-t border-white/[0.07] p-4"><div className="mx-auto max-w-3xl"><div className="flex items-end gap-2 rounded-xl border border-white/[0.1] bg-white/[0.03] p-2 focus-within:border-cyan-400/30"><textarea value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); void submit(); } }} rows={2} placeholder="询问当前企业项目、资料、规则、风险或研究问题…" className="min-h-12 flex-1 resize-none bg-transparent px-2 py-2 text-sm text-white outline-none placeholder:text-slate-600" />{query.length > 10_000 && <p className="mt-1 text-right text-[10px] text-amber-300">{query.length} 字：超过服务端 12,000 字上限的内容会被拒绝</p>}<button onClick={() => void submit()} disabled={!query.trim() || sending} aria-label="发送研判问题" className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-cyan-300 text-[#041018] disabled:opacity-40"><CornerDownLeft className="h-4 w-4" /></button></div></div></div>
        </>}
      </Panel>
