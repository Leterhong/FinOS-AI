...
function FactLedger({ facts, onReview }: { facts: EvidenceFact[]; onReview: (fact: EvidenceFact) => void }) {
  ...
  return <section className="mt-5"><p className="text-[10px] font-semibold uppercase tracking-[.15em] text-slate-600">结构化事实与原文引用</p><div className="mt-2 space-y-2">{facts.map((fact) => <article key={fact.id} className="rounded-xl border border-white/[0.07] p-3"><div className="flex flex-wrap items-center gap-2"><span className="text-xs text-slate-200">{fact.topic}</span><span className="numeric text-xs text-cyan-200">{fact.value}{fact.unit}</span><span className={`ml-auto text-[9px] ${fact.reviewStatus === "已确认" ? "text-emerald-300" : fact.reviewStatus === "已驳回" ? "text-rose-300" : "text-amber-300"}`}>{fact.reviewStatus}</span></div><p className="mt-2 text-[10px] leading-5 text-slate-500">“{fact.quote}”</p><div className="mt-2 flex items-center justify-between gap-3"><p className="text-[9px] text-slate-700">{fact.location || "位置未提供"}{fact.period ? ` · ${fact.period}` : ""}</p><button type="button" onClick={() => onReview(fact)} className="shrink-0 rounded-lg border border-white/10 px-2.5 py-1 text-[9px] text-slate-400 hover:border-cyan-400/20 hover:text-cyan-200">人工复核</button></div></article>)}</div></section>;
}
