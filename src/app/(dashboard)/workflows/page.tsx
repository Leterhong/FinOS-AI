...
import { EmptyStateCard, RiskBadge, PageIntro, Panel, PanelHeader } from "@/components/enterprise/EnterpriseUI";
...
export default function WorkflowsPage() {
  ...
  return <div className="page-shell"><PageIntro eyebrow="Human-in-the-loop" title="研判流程中心" description="将 Agent 发现转化为明确的补充资料、核验、复核与审批任务，让人机协作真正进入业务流程。" actions={<button onClick={() => setOpen(true)} className="inline-flex items-center gap-2 rounded-xl bg-cyan-300 px-4 py-2.5 text-xs font-semibold text-[#041018]"><Plus className="h-3.5 w-3.5" />创建任务</button>} />
    ...
    <Panel>
      <PanelHeader eyebrow="Pipeline" title="研判流程全景" description="从资料到决策的真实流转状态——每个节点的数字都来自当前工作区。" />
      <div className="flex flex-wrap items-center gap-2 p-5">
        {[
          { label: "资料上传", count: documents.length, href: "/documents" },
          { label: "AI 分析", count: documents.filter((d) => d.status === "已解析").length, href: "/documents" },
          { label: "规则匹配", count: documents.reduce((sum, d) => sum + (d.ruleHits || 0), 0), href: "/rules" },
          { label: "风险检出", count: risks.length, href: "/risk" },
          { label: "人工复核", count: risks.filter((r) => r.status === "待核验").length, href: "/risk" },
          { label: "决策交付", count: tasks.filter((t) => t.stage === "已完成").length, href: "/workflows" },
        ].map((node, index) => (
          <div key={node.label} className="flex items-center gap-2">
            <a href={node.href} className="group flex min-w-[104px] flex-col items-center rounded-xl border border-white/[0.07] bg-white/[0.02] px-4 py-3 transition hover:border-cyan-400/25">
              <span className="numeric text-xl font-semibold text-white">{node.count}</span>
              <span className="mt-0.5 text-[10px] text-slate-500 group-hover:text-cyan-200">{node.label}</span>
            </a>
