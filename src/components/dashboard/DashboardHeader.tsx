...
const titles: Record<string, string> = { "/": "企业经营决策台", "/cases": "项目中心", "/documents": "资料研判", "/risk": "风险中心", "/research": "投研中心", "/rules": "规则库", "/models": "AI 模型中心", "/agents": "Agent 中心", "/workflows": "流程中心", "/assistant": "智能研判助手" };
...
export default function DashboardHeader({ onMenuToggle }: { onMenuToggle?: () => void }) {
  ...
  return <header className="relative z-30 mb-5 flex h-[58px] shrink-0 items-center gap-3 border-b border-white/[0.07] pb-3">
    ...
    <div className="relative hidden xl:block"><button type="button" onClick={() => setWorkspaceOpen(value => !value)} className="flex items-center gap-2 text-xs text-slate-400"><span className="grid h-8 w-8 place-items-center rounded-lg border border-white/10 bg-white/[0.05] text-[10px] text-slate-200">FX</span><ChevronDown className="h-3 w-3" /></button>{workspaceOpen && <div className="absolute right-0 top-11 w-56 rounded-xl border border-white/10 bg-[#0a121e] p-2 shadow-2xl"><p className="px-2 py-2 text-[10px] text-slate-600">本地工作区 · 数据由你创建</p><button onClick={() => { if (window.confirm("确定清空当前工作区的项目、资料、规则和 AI 输出？")) clearWorkspace(); setWorkspaceOpen(false); }} className="w-full rounded-lg px-2 py-2 text-left text-xs text-rose-200/80 hover:bg-rose-400/[0.06]">清空本地工作区</button></div>}</div>
  </header>;
}
